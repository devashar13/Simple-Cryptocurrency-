...
node_address=str(uudid4()).replace('-','')
...
def connect_node():
    ...
    if nodes is None:
        return 'No node',400
    for node in nodes:
        blockchain.add(nodes)
    response={
    'message':'All the nodes are added and total number is :'
